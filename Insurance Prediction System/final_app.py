...
<style>
    ...
    /* ENHANCED Dark Mode Input Styling with white text - CRITICAL FIX */
    ...
    /* Number input arrows visibility */
    ...
    /* Alternative number input styling */
    ...
    /* COMBINED ACTION SECTION - Single container for both buttons */
    .combined-action-container {
        background: linear-gradient(135deg, #1a202c 0%, #2d3748 100%);
        padding: 2.5rem;
        border-radius: 20px;
        margin: 2rem 0;
...
# UPDATED Action Buttons Section - Combined Layout
st.markdown("""
<div class="section-header">
    🚀 Actions
</div>
...
# Combined container for both actions
st.markdown("""
<div class="combined-action-container">
    <h4>🔮 Premium Prediction & Data Management</h4>
    <div class="action-buttons-row">
        <div class="action-button-wrapper">
""", unsafe_allow_html=True)

# Create columns for the combined layout
col_predict, col_save = st.columns(2)

with col_predict:
    if st.button("🎯 Predict Insurance Premium", help="Get AI-powered insurance premium prediction", key="predict_btn"):
        # Get predicted premium for calculation
        base_premium = 12000
        age_factor = age * 80
        ...
        # Create DataFrame for immediate CSV save
        ...
        # Auto-save to CSV when predicting
        try:
            append_to_csv(EXCEL_FILE, new_data)
            csv_status = "✅ Auto-saved to CSV"
        except:
            csv_status = "⚠️ CSV save failed"
        
        st.markdown('</div>')  # Close combined container for display
        ...
        <div class="premium-display">
            ...
            <div style="font-size: 0.9rem; color: rgba(255, 255, 255, 0.8);">
                Based on your health profile | {csv_status}
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="background: rgba(102, 126, 234, 0.1); padding: 1rem; border-radius: 8px; margin-top: 1rem; border-left: 4px solid #667eea; color: #e2e8f0;">
            ...
            Actual premiums may vary based on additional medical examinations and insurer policies.<br>
            <strong>📁 Data Status:</strong> {csv_status} to {EXCEL_FILE}
        </div>
        """, unsafe_allow_html=True)

with col_save:
    if st.button("📁 Manual Save to CSV", help="Manually save data without prediction", key="save_btn"):
